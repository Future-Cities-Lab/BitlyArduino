...
void setup() {
  ...
  Ethernet.begin(mac, IPAddress(192,168,2,4));
  delay(200);
  ...
  for (int i = 0; i < 324; i++) {
    leds[i] = CRGB(135,206,250);
  }
  for (int i = 324; i < 648; i++) {
    leds[i] = CRGB(240,230,140);
  }
  FastLED.show();
}
...
void loop() {
  if (starting) {
    ...
    if (packetSize == IN_BUFFER_SIZE) {
      ...
      if (packetBuffer[0] == 4 && packetBuffer[1] == 1) {
        memcpy(leds, packetBuffer+2, 972);
      } else if (packetBuffer[0] == 4 && packetBuffer[1] == 2) {
