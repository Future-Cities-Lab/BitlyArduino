...
void setup() {
  ...
  Ethernet.begin(mac, IPAddress(192,168,2,9));
  delay(200);
  ...
  for (int i = 0; i < 324; i++) {
    leds[i] = CRGB(176,48,96);
  }
  for (int i = 324; i < 648; i++) {
    leds[i] = CRGB(105,105,105);
  }
  FastLED.show();
}
...
void loop() {
  ...
  int packetSize = Udp.parsePacket();
    if (packetSize == IN_BUFFER_SIZE) {
      ...
      if (packetBuffer[0] == 4 && packetBuffer[1] == 3) {
        memcpy(leds, packetBuffer+2, 972);
      } else if (packetBuffer[0] == 4 && packetBuffer[1] == 4) {
